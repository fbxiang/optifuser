...
void OptixRenderer::init() {
  ...
  optix::Buffer output_buffer =
      context->createBufferFromGLBO(RT_BUFFER_INPUT_OUTPUT, vbo);
  ...
  context->setExceptionProgram(
      0, context->createProgramFromPTXFile(ptxFile, "exception"));
  context->setRayGenerationProgram(
      0, context->createProgramFromPTXFile(ptxFile, "camera"));

  // ptxFile = get_ptx_filename("environment_map");
  // context->setMissProgram(0, context->createProgramFromPTXFile(ptxFile,
  // "miss")); context["envmap"]->setTextureSampler();
...
void OptixRenderer::initSceneGeometry(std::shared_ptr<Scene> scene) {
  ...
  if (auto envMap = scene->getEnvironmentMap()) {
    ...
    context->setMissProgram(0,
                            context->createProgramFromPTXFile(ptxFile, "miss"));
    printf("Finishing\n");
  } else {
...
void OptixRenderer::renderScene(std::shared_ptr<Scene> scene) {
  ...
  for (auto obj : scene->getObjects()) {
    ...
    if (transform) {
      transform->getChild<optix::GeometryGroup>()
          ->getAcceleration()
          ->markDirty();
    }
  }
...
optix::Transform
OptixRenderer::getObjectTransform(std::shared_ptr<Object> obj) {
...
optix::Geometry
OptixRenderer::getMeshGeometry(std::shared_ptr<TriangleMesh> mesh) {
  auto p = _mesh_geometry.find(mesh);
  optix::Geometry g = 0;
  if (p == _mesh_geometry.end()) {
    optix::Buffer vertices =
        context->createBufferFromGLBO(RT_BUFFER_INPUT, mesh->getVBO());
    ...
    optix::Buffer indices =
        context->createBufferFromGLBO(RT_BUFFER_INPUT, mesh->getEBO());
    ...
    if (!_mesh_intersect) {
      ...
      _mesh_intersect =
          context->createProgramFromPTXFile(ptxFile, "mesh_intersect");
...
optix::Geometry
OptixRenderer::getMeshGeometry(std::shared_ptr<DynamicMesh> mesh) {
...
OptixRenderer::getTextureSampler(std::shared_ptr<Texture> tex) {
  if (tex->getId() == 0)
    return 0;
  ...
  if (p == _texture_sampler.end()) {
    sampler = context->createTextureSamplerFromGLImage(tex->getId(),
...
optix::TextureSampler OptixRenderer::getEmptySampler() {
  if (!_empty_sampler) {
    ...
    optix::Buffer buffer = context->createBuffer(
        RT_BUFFER_INPUT, RT_FORMAT_UNSIGNED_BYTE4, 1u, 1u);
    _empty_sampler->setBuffer(0u, 0u, buffer);
  }
  return _empty_sampler;
}

void OptixRenderer::renderSceneToFile(std::shared_ptr<Scene> scene,
                                      std::string filename) {
  if (iterations >= max_iterations)
    return;
