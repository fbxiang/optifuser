...
void renderObjectTree(const Object &obj, const glm::mat4 &parentModelMat, const glm::mat4 &viewMat,
  ...
  for (auto &child : obj.getChildren()) {
    renderObjectTree(*child, modelMat, viewMat, viewMatInv, projMat, projMatInv, defaultShader,
                     renderSegmentation);
  }
}
...
void GBufferPass::render(const Scene &scene, const CameraSpec &camera,
  ...
  for (const auto &obj : scene.getObjects()) {
    renderObjectTree(*obj, glm::mat4(1.f), viewMat, viewMatInv, projMat, projMatInv,
                     m_shader.get(), renderSegmentation);
  }
}
